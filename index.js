...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Adrx\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558197660171:+55558197660171 \n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('connecting', () => {
		start('2', 'ESPERE UM POUCO....')
	})
	client.on('open', () => {
		success('2', 'TAMO ON FML')
	})
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ANA_BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Adrx domina vida
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "AkTr", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'Ctt do meu dono ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
                       client.sendMessage(from, {displayname: "Adrx", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'Ctt do meu dono ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o AkTr!' ,text, { quoted: mek })
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* AkTr & Adrx\n*YOUTUBE:* ....\n*WPP:* wa.me/558896668696 wa.me/558197660171/n*INSTA:* ...\n\n\nEspero que tenham gostado do bot 🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Quem é Você?, Voce não é meu dono kkk')
					...
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você, você não é meu dono kkkk?')
